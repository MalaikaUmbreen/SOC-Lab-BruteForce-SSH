...
Produce a job-level SOC Incident Report suitable for interviews and portfolio
---
⚔️ Attack Execution
Tool Used
...
`22:01:57`	Third run with `-V` flag — password cracked successfully
`22:06:43`	Fourth run — wrong password, failed
`22:06:48`	Final run finished
---
📸 Evidence Screenshots
1. Hydra Attack from Kali Linux Terminal
![Hydra Attack](screenshots/hydra-attack-kali.png)
> THC-Hydra successfully cracking the SSH password for user `malaika`
---
2. Wazuh SIEM — 28 Alerts Triggered
![Wazuh Dashboard](screenshots/wazuh-siem-alert.png)
...
Was it successful?	✅ Yes — password cracked
...
Block attacker IP via firewall rules immediately
Disable password-based SSH — use key-pair authentication only
Enforce strong password policy — minimum 12 characters, mixed complexity
Enable account lockout after 3–5 failed attempts (Fail2Ban or PAM tally)
Enable MFA for all SSH-accessible accounts
Change default SSH port from 22 to a non-standard port
Configure Wazuh Active Response to auto-block IPs on brute force detection
Rotate all credentials and conduct a full account audit
---
...
│
└── screenshots/
    ├── hydra-attack-kali.png                     # Hydra terminal output
    ├── wazuh-siem-alert.png                      # Wazuh 28 events
    └── auth-log-victim.png                       # Ubuntu auth.log evidence
```
---
🧰 Skills Demonstrated
